...
def fetch_kluster(key):
    data = _get(
        "https://api.kluster.ai/v1/models", headers={"Authorization": f"Bearer {key}"}
    )
    return [
        {
            "id": m["id"],
...
def fetch_zai(key):
    ...
    out = [
        {"id": mid, "name": name, "context": None, "limits": "free Flash tier"}
...
<div class="layout" id="layout">
  ...
  <main>
    ...
    <div class="suggest-card">
      ...
      <a class="suggest-btn"
         href="https://github.com/Danissimode/litellm-free-models-prox-DNy/issues/new?template=new-provider.yml"
...
<footer>
  <p>Auto-generated by <a href="https://github.com/Danissimode/litellm-free-models-prox-DNy">litellm-free-models-proxy</a> · Cross-referenced with <a href="https://github.com/cheahjs/free-llm-api-resources">cheahjs/free-llm-api-resources</a>. Free-tier API tokens — not open-source or self-hostable models. Tiers may change without notice; not affiliated with any provider.</p>
</footer>
</div>
<script>
...
def render_cross_provider(groups, provider_map):
    ...
    for cname, entries in groups:
        ...
        html += (
            f'<div class="cross-group" data-total="{rows_count}">{header}{table}</div>'
        )
    return html
    ...
    for entry in reversed(history):
        ...
        if added:
            summary_parts.append(
                f'<span style="color:#22c55e">+{len(added)} added</span>'
            )
        if removed:
            summary_parts.append(
                f'<span style="color:#f87171">−{len(removed)} removed</span>'
            )
...
def render_availability(provider_list, results, availability):
    ...
    if not availability:
        return (
            '<p class="av-empty" style="text-align:center;padding:2rem">'
    ...
    for p in provider_list:
        ...
        for m in sorted(models, key=sort_key):
            ...
            tag_labels = escape(
                " ".join(
                    label
                    for label, _ in get_tags(
                        mid, m.get("context"), m.get("capabilities")
                    )
                )
            )
            ctx_raw = int(m.get("context") or 0)
...
def main():
    ...
    try:
        ...
        for pk, models in list(results.items()):
            ...
            if dropped:
                print(
                    f"  [{pk}] dropped {len(dropped)} models with sustained 0% uptime: "
        ...
        if dropped_total:
            print(
                f"  Self-correcting layer dropped {dropped_total} broken models total"
            )
    except Exception as e:
    ...
    for slug, tab, base, av in entry_points:
        ...
        sub_html = html.replace("<head>", f'<head>\n<base href="{base}">', 1).replace(
            "<body>", f"<body {body_attrs}>", 1
        )
